...
export class CryptoDetailedAccount extends RegistryItem {
  ...
  public getTokenIds = () => {
    ...
    this.tokenIds.map(tokenId => {
      if (tokenId instanceof HexString) return tokenId.getStringData();
      return tokenId;
    })
  }
  ...
  public toDataItem = () => {
    ...
    const account = this.account.toDataItem();
    account.setTag(this.account.getRegistryType().getTag());
    map[Keys.account] =  account;
  ...
  public static fromDataItem = (dataItem: DataItem) => {
    ...
    if (map[Keys.tokenIds]) {
      ...
      tokenIds = tokenIdsRead.map( (tokenId) => {
        ...
        // Check if hex string
        else if(typeof tokenId == typeof DataItem) {
