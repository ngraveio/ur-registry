...
describe("CryptoPortfolioMetadata", () => {
    ...
    it("should encode Portfolio Metadata with addition properties", () => {
      ...
      const expectedHex = "ab0150babe0000babe001122334455667788990262656e0365312e302e3004696d792d64657669636566737472696e676b68656c6c6f20776f726c64666e756d626572187b67626f6f6c65616ef565617272617983010203666f626a656374a2616101616202646e756c6cf66464617465c11a5fee6600";
      expect(cbor).toBe(expectedHex);
      expect(ur.toString()).toBe("ur:portfolio-metadata/pyadgdrdrnaeaerdrnaebycpeofygoiyktlonlaoidihjtaxihehdmdydmdyaainjnkkdpieihkoiniaihiyjkjyjpinjtiojeisihjzjzjlcxktjljpjzieiyjtkpjnidihjpcskgioidjljljzihhsjtykihhsjpjphskklsadaoaxiyjlidimihiajyoehshsadhsidaoiejtkpjzjzynieiehsjyihsecyhewyiyaeahhngoeo");
    });
...
describe("CryptoPortfolioMetadata language codes", () => {
    ...
    it("should throw error encoding with incorrect language codes", () => {
        expect(() => {
            //@ts-ignore
            const metadata = new PortfolioMetadata({"language": "xx"});
        }).toThrowError("Invalid language code");
        expect(() => {
            //@ts-ignore
            const metadata = new PortfolioMetadata({"language": "xyx"});
        }).toThrowError("Invalid language code");
    });
    
    it("should throw error decoding CBOR with incorrect language codes", () => {
        expect(() => {
            PortfolioMetadata.fromHex("a401f7026378797a03f704f7");
        }).toThrowError("Invalid language code");
    });
